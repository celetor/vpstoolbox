...
installdependency(){
  ...
  if [[ $dist = centos ]]; then
    ...
    apt-get install sudo curl socat xz-utils wget apt-transport-https gnupg gnupg2 dnsutils lsb-release python3-qrcode python-pil unzip resolvconf ntpdate systemd dbus -qq -y
 else
  clear
...
touch /etc/nginx/conf.d/trojan.conf || true
  if [[ $dist != centos ]]; then
    nginxconf
 else
  TERM=ansi whiptail --title "continuing..." --infobox "continuing..." 8 78
...
removenginx(){
  ...
  systemctl disable nginx || true
    if [[ $dist = centos ]]; then
    yum remove nginx dnsmasq qbittorrent-nox -y || true
    else
    apt purge nginx dnsmasq qbittorrent-nox -p -y || true
    rm -rf /etc/apt/sources.list.d/nginx.list || true
  fi
  sudo ~/.acme.sh/acme.sh --uninstall || true
}
##########Check for update############
checkupdate(){
  cd
...
$v2link
EOF
  cp /etc/v2ray/$uuid.txt /usr/share/nginx/html/ || true
